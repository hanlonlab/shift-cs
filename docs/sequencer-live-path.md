...
Submission ingress uses the version 1 canonical frame. Producers send exactly one frame of at most 2,048 bytes per AF_UNIX datagram with a defined `MessageType`, a nonzero `ProducerId`, a nonzero `ProducerSequence`, and `SequenceId = 0`. Producer ID 0 is reserved for Archiver control frames (`CommitThrough`). The Sequencer preserves the message type, producer identity, and payload, assigns the session sequence, and regenerates the frame checksum. It does not add a timestamp. Time that changes deterministic business state must arrive as an explicit sequenced command.

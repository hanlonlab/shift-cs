...
Owns the single ordering loop. It deduplicates submissions, assigns the session sequence, batches candidates for the Archiver, and multicasts only durably committed frames.
...
A batch begins with the first new submission and closes after at most 1 ms, at 1 MiB of canonical frame bytes, or immediately on `EndCurrentSession`. The deadline does not move when more submissions arrive. Only one batch may await the Archiver. The Sequencer publishes the batch frames in sequence order and then its `CommitThrough` watermark after the Archiver returns the exact expected durable high-water mark.

...
The deployed archive root is `/var/lib/shift/archive`; `/run/shift` remains reserved for transient sockets. An empty-payload `StartNewSession` creates `{SessionId:N}.shiftlog` from its header session ID with `FileMode.CreateNew`. Every later frame must carry that session ID. `EndCurrentSession` must be the last frame in its batch; after that batch is synced, the file is closed before a `CommitThrough` carrying the same session ID is sent. The parent directories must already exist.
